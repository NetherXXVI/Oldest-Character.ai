...
or equal to the number of stages ({self._num_stages})."
    ...
    def step(self, *args, target=None, losses: Optional[list] = None, **kwargs):
        ...
        # Clean per iteration
        logits_to_keep = kwargs['logits_to_keep'] if 'logits_to_keep' in kwargs else None
        batch_seqlen = kwargs['batch_seqlen'] if "batch_seqlen" in kwargs else None
        self._stage.clear_runtime_states(batch_seqlen=batch_seqlen, logits_to_keep=logits_to_keep)
        ...
        if self._stage.is_last and self._stage.return_outputs:
...
class PipelineScheduleMulti(_PipelineSchedule):
    ...
    def step(self, *args, target=None, losses: Optional[list] = None, **kwargs):
        ...
        batch_seqlen = kwargs['batch_seqlen'] if "batch_seqlen" in kwargs else None
        for stage in self._stages:
            stage.clear_runtime_states(batch_seqlen=batch_seqlen, logits_to_keep=logits_to_keep)
        ...
        # Return merged results per original format
        for stage in self._stages:
            if stage.is_last and stage.return_outputs:
