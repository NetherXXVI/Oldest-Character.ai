...
class _PipelineStageBase(ABC):
    ...
    def clear_runtime_states(self, batch_seqlen=None, logits_to_keep=None) -> None:
        """
        ...
        for recv_tuple in self.args_recv_info.values():  # iterate over all chunks
            for a in recv_tuple:  # iterate over all input args
                if isinstance(a, _RecvInfo):
                    ...
                    a.buffer.grad = None
                    orig = a.buffer
                    # Check if current shape needs to change
                    # if new_seqlen is not None and a.buffer.shape[1] != new_seqlen:
                    if batch_seqlen is not None:
                        # Allocate a new buffer with the desired shape
                        dtype = a.buffer.dtype
                        device = a.buffer.device
                        requires_grad = a.buffer.requires_grad
                        a.buffer = torch.empty((a.buffer.shape[0], batch_seqlen, a.buffer.shape[-1]),
                                            dtype=dtype,
                                            device=device,
        ...
        if self._outputs_meta is not None and batch_seqlen is not None:
            new_output_meta = []
            for meta_tensor in self._outputs_meta:
                orig_shape = meta_tensor.shape
                if len(orig_shape) == 3:
                    if self.is_last and logits_to_keep is not None:
                        ## this is for logits and logits to keep
                        if isinstance(logits_to_keep, torch.Tensor):
                            new_shape = (meta_tensor.shape[0], 1, meta_tensor.shape[-1])
                        else:
                            new_shape = (meta_tensor.shape[0], logits_to_keep, meta_tensor.shape[-1])
                        new_output_meta.append(torch.empty(new_shape, device=meta_tensor.device, dtype=meta_tensor.dtype))
                    else:
                        ## this is for regular case need to div by sp_size
                        new_shape = (meta_tensor.shape[0], batch_seqlen, meta_tensor.shape[-1])
                        new_output_meta.append(torch.empty(new_shape, device=meta_tensor.device, dtype=meta_tensor.dtype))
        ...
        ## now reset grad recvs
        for recv_tuple in self.grad_recv_info.values():  # iterate over all chunks
            for a in recv_tuple:  # iterate over all input args
                if isinstance(a, _RecvInfo):
                    # Set to None is the newer and recommended way to clear grads, compared to `zero_()`.
                    # See https://github.com/pytorch/pytorch/pull/92731
    ...
    def forward_one_chunk(
        ...
        if self.is_last and self.return_outputs:
